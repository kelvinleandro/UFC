...
md`# Visualização de Crimes em Chicago usando Vega-Lite - 2025.2 v2.0`
)}
...
FileAttachment("chicago_crimes_october_2025@1.csv").csv({typed: true}).then(function (data) {
  ...
  data.forEach(function(d) {
    ...
    const hours = d.Date.getUTCHours().toString().padStart(2, "0");
    const minutes = d.Date.getUTCMinutes().toString().padStart(2, "0");
...
async function _crimechart(vl,topo,crimes,width)
{
    ...
    const colorScale = vl.color().fieldN('Primary Type')
      .scale({ domain: crimeTypes, range: typeColors });
    ...
    const pointsLayer = vl.markCircle({opacity: 0.7, stroke: 'black', strokeWidth: 0.3})
      .data(crimes)
      .transform(vl.filter({param: 'crimeSelect'}))
      .encode(
        ...
        vl.tooltip([
          ...
          vl.fieldT('Date', {timeUnit: 'yearmonthdate', title: 'Date'}),
          vl.fieldN('time', {title: 'Time'}),
        ])
      );
    ...
    .data(crimes)
    .params({ name: 'crimeSelect', select: { type: 'point', fields: ['Primary Type'] } })
    .encode(
    ...
    const element = await vl.hconcat(crimeMap, rightPanel)
      .resolve({ scale: { color: 'shared' } })
      .title({ text: `Crimes in Chicago in October 2025`, fontSize: 28 })
      .render();
    
    // Acessar a view do Vega a partir do elemento DOM
    const view = element.value;
    
    // Adicionar listener para mudanças na seleção
    let selectedCrime = null;
    view.addSignalListener('crimeSelect', (name, value) => {
      selectedCrime = value && value['Primary Type'] && value['Primary Type'].length > 0 
        ? value['Primary Type'][0] 
        : null;
      element.value = selectedCrime;
...
function _displayInfo(crimechart)
{
  let content = "None selected";
  if(crimechart){
    content = crimechart || "None selected";
  }
  return content;
}


function _table(Inputs,crimechart,crimes){return(
Inputs.table(crimechart ? crimes.filter(c => c['Primary Type'] === crimechart): crimes)
)}
...
export default function define(runtime, observer) {
  ...
  main.variable(observer("viewof crimechart")).define("viewof crimechart", ["vl","topo","crimes","width"], _crimechart);
  main.variable(observer("crimechart")).define("crimechart", ["Generators", "viewof crimechart"], (G, _) => G.input(_));
  main.variable(observer("displayInfo")).define("displayInfo", ["crimechart"], _displayInfo);
  main.variable(observer("viewof table")).define("viewof table", ["Inputs","crimechart","crimes"], _table);
