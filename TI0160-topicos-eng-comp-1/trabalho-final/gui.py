...
try:
    ...
    print(
        "AVISO: Módulo 'histogram_cuda' não encontrado. Compile com 'python build.py'."
    )
...
class HistogramApp:
    def __init__(self, root: tk.Tk):
        self.root = root
        self.root.title("Visualizador de Histograma via CUDA")
        self.root.geometry("1000x600")

        # 'header' que ficará o botão
        ...
        self.right_frame = tk.LabelFrame(
            self.main_frame, text="Histograma (CUDA)", width=500
        )
    ...
    def load_image(self):
        if not CUDA_AVAILABLE:
            messagebox.showerror(
                "Erro", "Módulo CUDA não encontrado!\nRode 'python build.py' primeiro."
            )
            return
        ...
        # Carregar imagem para visualização (BGR -> RGB)
        ...
        # Processamento CUDA
        try:
            hist_data = histogram_cuda.calculate_histogram(img_gray)
        except Exception as e:
            messagebox.showerror("Erro CUDA", f"Erro no processamento:\n{str(e)}")
            return
    ...
    def display_image(self, cv_img):
        ...
        # Atualizar label (precisa manter referência para não ser coletado pelo GC)
        self.lbl_image.config(image=tk_img, text="")
        self.lbl_image.image = tk_img

    def plot_histogram(self, hist_data):
        self.ax.clear()
        self.ax.bar(range(256), hist_data, color="#444444", width=1.0)
